 ...
 *   Examples: bitSum16bit(257) = 0, bitSum8bit(511) = 1
 *   Legal ops: & ( ) >> + 
 *   Do not use a loop!
 */
...
{
	int mask = 0x5555;
	int perEightParity = (input & mask) ^ ((input >> 1) & mask); 
	int mask2 = 0x1111;
	int quarterParity = (perEightParity & mask2) ^ ((perEightParity >> 2) & mask2);
	int mask3 = 0x0101;
	int halfParity = (quarterParity & mask3) ^ ((quarterParity >> 4) & mask3);
	int mask4 = 0x01
	return (halfParity & mask4) ^ ((halfParity >> 4) & mask4);
}
